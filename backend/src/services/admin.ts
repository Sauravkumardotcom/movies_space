...
export const adminService = {
  ...
  async getPlatformStats() {
    try {
      const [totalUsers, totalMovies, totalMusic, totalShorts, totalComments, totalRatings] =
        await Promise.all([
          prisma.user.count(),
          prisma.movie.count(),
          prisma.music.count(),
   ...
   */
  async deleteCommentAdmin(commentId: string, reason?: string) {
    try {
   ...
   */
  async reportContent(userId: string, contentId: string, contentType: string, reason: string) {
    try {
