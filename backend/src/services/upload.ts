...
export const uploadService = {
  ...
  validateFile(mimeType: string, fileSize: number) {
    ...
    if (fileSize > MAX_FILE_SIZE) {
      throw new Error(
        `File size exceeds limit: ${(fileSize / 1024 / 1024).toFixed(2)}MB / ${(MAX_FILE_SIZE / 1024 / 1024).toFixed(2)}MB`
      );
    }
  ...
  async createUpload(userId: string, input: UploadCreateInput) {
    try {
  ...
  async updateUploadStatus(id: string, userId: string, input: UploadStatusUpdateInput) {
    try {
  ...
  async getUploadStats(userId: string) {
    try {
      ...
      const stats = {
        ...
        totalSizeMB: Number(
          (uploads.reduce((acc, u) => acc + u.fileSize, 0) / 1024 / 1024).toFixed(2)
        ),
      };
