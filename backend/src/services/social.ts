...
export const socialService = {
   ...
   * Check if following
   */
  async isFollowing(userId: string, followId: string) {
    try {
   ...
   * Add item to list
   */
  async addItemToList(userId: string, listId: string, entityId: string, entityType: string) {
    try {
   ...
   */
  async removeItemFromList(userId: string, listId: string, entityId: string, entityType: string) {
    try {
      // Verify list ownership
      const list = await prisma.list.findUnique({
        where: { id: listId },
