...
export const musicService = {
  ...
  async getMusic(filters: MusicFilters): Promise<PaginatedResponse<any>> {
    try {
      const validated = musicFilterSchema.parse(filters);
      const skip = (validated.page - 1) * validated.limit;

      const where: any = {};
      if (validated.artist) where.artist = { contains: validated.artist, mode: 'insensitive' };
      if (validated.genre) where.genre = { contains: validated.genre, mode: 'insensitive' };
  ...
  // Search music
  async searchMusic(query: string, page: number = 1, limit: number = 20) {
    try {
      if (query.length < 2) {
  ...
  // Create playlist
  async createPlaylist(userId: string, input: PlaylistCreateInput) {
    try {
  ...
  async removeSongFromPlaylist(playlistId: string, musicId: string, userId: string) {
    try {
      // Verify ownership
      const playlist = await prisma.playlist.findUnique({
        where: { id: playlistId },
