...
router.get('/trending', async (req, res) => {
  try {
    ...
    const data = await musicService.getTrending(parseInt(limit as string) || 10);
...
router.get('/:id', async (req, res) => {
  try {
    ...
    musicService.incrementPlayCount(id).catch(err =>
      logger.error('Error incrementing play count:', err)
    );
...
router.get('/playlists', authMiddleware, async (req, res) => {
  try {
    ...
    const result = await musicService.getUserPlaylists(
      userId,
      parseInt(page as string) || 1,
      parseInt(limit as string) || 20
    );
