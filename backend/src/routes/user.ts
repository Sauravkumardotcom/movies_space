 ...
 */
router.delete('/watchlist/:movieId', authMiddleware, async (req: AuthRequest, res: Response) => {
  try {
    if (!req.user?.userId) {
      sendResponse(res, 401, 'Unauthorized');
      return;
...
router.delete(
  ...
  async (req: AuthRequest, res: Response) => {
    try {
      ...
      await userActivityService.removeFromFavorites(
        req.user.userId,
        req.params.entityId,
...
router.post('/history', authMiddleware, async (req: AuthRequest, res: Response) => {
  try {
    ...
    await userActivityService.addToHistory(
      req.user.userId,
      entityId,
      entityType,
      progress,
 ...
 */
router.post('/ratings', authMiddleware, async (req: AuthRequest, res: Response) => {
  try {
    if (!req.user?.userId) {
      sendResponse(res, 401, 'Unauthorized');
      return;
    ...
    const result = await userActivityService.addRating(
      req.user.userId,
      entityId,
      entityType,
      rating,
...
router.delete(
  ...
  async (req: AuthRequest, res: Response) => {
    try {
      ...
      await userActivityService.removeRating(
        req.user.userId,
        req.params.entityId,
        req.params.entityType
      );
