...
router.get('/lists/:listId', async (req: Request, res: Response) => {
  try {
    ...
    const result = await socialService.getList(
      listId,
      parseInt(page as string) || 1,
      parseInt(limit as string) || 20
    );
 ...
 * Get user's lists
 */
router.get('/user-lists/:userId', async (req: Request, res: Response) => {
  try {
    const { userId } = req.params;
    const { page = 1, limit = 20 } = req.query;

    const result = await socialService.getUserLists(
      userId,
      parseInt(page as string) || 1,
      parseInt(limit as string) || 20
    );
 ...
 * Update list
 */
router.put('/lists/:listId', authMiddleware, async (req: Request, res: Response) => {
  try {
    const { listId } = req.params;
    const { name, description, isPublic } = req.body;
    const userId = (req as any).user.userId;
 ...
 * Delete list
 */
router.delete('/lists/:listId', authMiddleware, async (req: Request, res: Response) => {
  try {
    const { listId } = req.params;
    const userId = (req as any).user.userId;
 ...
 * Add item to list
 */
router.post('/lists/:listId/items', authMiddleware, async (req: Request, res: Response) => {
  try {
    const { listId } = req.params;
 ...
 */
router.delete('/lists/:listId/items/:entityId/:entityType', authMiddleware, async (req: Request, res: Response) => {
  try {
