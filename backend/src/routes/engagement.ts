 ...
 */
router.post('/ratings', authMiddleware, async (req: Request, res: Response) => {
  try {
...
router.get('/favorites', authMiddleware, async (req, res) => {
  try {
    ...
    const result = await engagementService.getUserFavorites(
      userId,
...
router.get('/watchlist', authMiddleware, async (req, res) => {
  try {
    ...
    const result = await engagementService.getUserWatchlist(
      userId,
      parseInt(page as string) || 1,
      parseInt(limit as string) || 20
    );
...
router.get('/history', authMiddleware, async (req, res) => {
  try {
    ...
    const result = await engagementService.getUserHistory(
      userId,
      entityType as string | undefined,
      parseInt(page as string) || 1,
      parseInt(limit as string) || 20
...
router.get('/history/:entityId/:entityType', authMiddleware, async (req, res) => {
  try {
    ...
    const progress = await engagementService.getWatchProgress(
      userId,
      entityId,
      entityType
    );
